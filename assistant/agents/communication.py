...
class CommunicationAgent(Agent):
    ...
    def suggest_replies(self, context, count=3):
        ...
        address = (f"Обращаться можно только по имени «{peer_name.split()[0]}» "
                   "или без обращения; других имён не выдумывай. "
                   if peer_name else "")
        system = (
            ...
            f"переписки: длину, тон, обращения. {address}"
            "Никаких пояснений и подписей."
        )
        user = ("\n\n".join(blocks) + "\n\nДай ровно "

...
* `/v1/*` — приём пакетов: Health Auto Export шлёт сюда все метрики сам,
  агент на Mac — звонки и переписку. Обычный HTTP, потому что так умеют
  и приложение, и простой `curl`.
...
Полная история Здоровья и базы Apple забираются командами
`python -m phone import-health` / `mac-sync`, не через этот вход: файл
выгрузки — гигабайт, и тащить его POST-ом незачем.

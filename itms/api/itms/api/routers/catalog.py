...
async def create_manufacturer(
    payload: ManufacturerWrite, session: SessionDep
) -> ManufacturerRead:
...
@router.get("/models", response_model=Page[DeviceModelRead],
            dependencies=[requires(Permission.CI_READ)])
...
@router.post("/models", response_model=DeviceModelRead, status_code=201,
             dependencies=[requires(Permission.CATALOG_WRITE)])
...
@router.patch("/models/{model_id}", response_model=DeviceModelRead,
              dependencies=[requires(Permission.CATALOG_WRITE)])
...
@router.post("/models/{model_id}/port-templates", response_model=PortTemplateRead,
             status_code=201, dependencies=[requires(Permission.CATALOG_WRITE)])
...
@router.delete("/port-templates/{template_id}", response_model=Ok,
               dependencies=[requires(Permission.CATALOG_WRITE)])
